...
import { useHostsContext } from "../list/state/context";
...
const FormHost: FC<Props> = ({ toggle, type = EFormProduct.new }) => {
  const [form] = useForm();
  const [loading, setLoading] = useState<boolean>(false);


  const { pageSize, page, search, fetchDataCallback } = useHostsContext();


  const createHost = async (values: FormData) => {
    try {
      setLoading(true);
      const response: any = await hostApi.add(values);
